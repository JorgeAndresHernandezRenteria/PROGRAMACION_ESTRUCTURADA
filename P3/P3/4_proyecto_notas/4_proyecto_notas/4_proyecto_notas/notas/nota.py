...
def crear(usuario_id,titulo,descripcion): #Le mando los parametros
    try:
...
def cambiar(id,titulo,descripcion):
  try:
    conexion.execute("update notas set titulo=%s,descripcion=%s,fecha=now() where id=%s",(titulo,descripcion,id))
    conexion.commit()
    return True
  except:
    return False
...
def borrar(id):
   try:
    conexion.execute("delete from notas where id=%s",(id,))
    conexion.commit()
    return True
   except:
    return False